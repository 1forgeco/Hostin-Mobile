...
function OwnerDashboardSkeleton({ isCompact }: { isCompact: boolean }) {
  return <View style={styles.skeletonWrap}>
    <Text style={[styles.skeletonTitle, isCompact && styles.skeletonTitleCompact]}>Preparing your dashboard...</Text>
    <View style={styles.skeletonStatus}><Ionicons color="#6D5CF6" name="sparkles" size={16} /><Text style={styles.skeletonStatusText}>Syncing today's operations</Text></View>
    <View style={styles.skeletonMetricGrid}>
      {[0, 1, 2, 3].map((item) => <View key={item} style={styles.skeletonMetricCard}>
    ...
    <View style={styles.skeletonSectionHeader}><Text style={styles.ownerSectionTitle}>Quick access</Text><Text style={styles.skeletonLink}>View all</Text></View>
    <View style={styles.skeletonQuickGrid}>{[0, 1, 2, 3].map((item) => <View key={item} style={styles.skeletonQuickCard}><View style={styles.skeletonQuickIcon} /><View style={styles.skeletonQuickLine} /></View>)}</View>
    <View style={styles.skeletonSectionHeader}><Text style={styles.ownerSectionTitle}>Needs attention</Text><Text style={styles.skeletonLink}>View all</Text></View>
    <View style={styles.skeletonList}>{[0, 1, 2].map((item) => <View key={item} style={[styles.skeletonListRow, item < 2 && styles.skeletonListBorder]}><View style={styles.skeletonSmallAvatar} /><View style={styles.skeletonListCopy}><View style={styles.skeletonListTitle} /><View style={styles.skeletonListText} /></View><View style={styles.skeletonBadge} /></View>)}</View>
  </View>;
}
...
export default function DashboardScreen() {
  ...
  const ownerFloating = role === 'owner' && data ? <View pointerEvents="box-none" style={styles.floatingLayer}>
  ...
  if (role === 'owner' && session) {
    ...
    return <Screen contentStyle={[styles.ownerContent, isCompact && styles.ownerContentCompact]} floating={ownerFloating} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void refresh(true)} tintColor={colors.forest} />}>
      ...
      {searchModal}
      {ownerLoading ? <OwnerDashboardSkeleton isCompact={isCompact} /> : <>
...
const styles = StyleSheet.create({
  ...
  skeletonWrap: { paddingTop: 16 }, skeletonTitle: { color: colors.ink, fontSize: 21, fontWeight: '900', letterSpacing: 0 }, skeletonTitleCompact: { fontSize: 19 }, skeletonStatus: { alignItems: 'center', flexDirection: 'row', gap: 8, marginTop: 12 }, skeletonStatusText: { color: '#716A9A', fontSize: 13, fontWeight: '700' }, skeletonMetricGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginTop: 28 }, skeletonMetricCard: { backgroundColor: 'rgba(255,255,255,0.72)', borderColor: 'rgba(219,216,245,0.84)', borderRadius: 15, borderWidth: 1, height: 128, padding: 16, width: '48%' }, skeletonAvatar: { backgroundColor: '#E7E4F8', borderRadius: 24, height: 48, width: 48 }, skeletonLineWide: { backgroundColor: '#E1DEF3', borderRadius: 8, height: 13, left: 76, position: 'absolute', top: 28, width: '34%' }, skeletonLineShort: { backgroundColor: '#E1DEF3', borderRadius: 8, height: 13, position: 'absolute', right: 18, top: 28, width: 42 }, skeletonLineMid: { backgroundColor: '#DFDCF1', borderRadius: 8, height: 11, left: 18, position: 'absolute', right: 28, top: 86 }, skeletonSectionHeader: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10, marginTop: 26 }, skeletonLink: { color: '#6D5CF6', fontSize: 12, fontWeight: '900' }, skeletonQuickGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 9 }, skeletonQuickCard: { alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.72)', borderColor: 'rgba(219,216,245,0.84)', borderRadius: 14, borderWidth: 1, height: 92, justifyContent: 'center', width: '22.5%' }, skeletonQuickIcon: { backgroundColor: '#E7E4F8', borderRadius: 22, height: 44, width: 44 }, skeletonQuickLine: { backgroundColor: '#E1DEF3', borderRadius: 7, height: 10, marginTop: 13, width: 50 }, skeletonList: { backgroundColor: 'rgba(255,255,255,0.72)', borderColor: 'rgba(219,216,245,0.84)', borderRadius: 15, borderWidth: 1, overflow: 'hidden' }, skeletonListRow: { alignItems: 'center', flexDirection: 'row', gap: 12, minHeight: 64, paddingHorizontal: 14 }, skeletonListBorder: { borderBottomColor: '#E5E2F1', borderBottomWidth: 1 }, skeletonSmallAvatar: { backgroundColor: '#E7E4F8', borderRadius: 18, height: 36, width: 36 }, skeletonListCopy: { flex: 1, gap: 8 }, skeletonListTitle: { backgroundColor: '#E1DEF3', borderRadius: 7, height: 11, width: '58%' }, skeletonListText: { backgroundColor: '#E1DEF3', borderRadius: 7, height: 10, width: '86%' }, skeletonBadge: { backgroundColor: '#E1DEF3', borderRadius: 8, height: 12, width: 52 },
